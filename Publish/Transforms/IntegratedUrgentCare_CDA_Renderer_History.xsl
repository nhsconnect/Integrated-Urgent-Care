...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3" xmlns:npfitlc="NPFIT:HL7:Localisation" xmlns:n2="urn:hl7-org:v3/meta/voc" xmlns:voc="urn:hl7-org:v3/voc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xhtml="NPfIT:PresentationText"  exclude-result-prefixes="n1 n2 voc npfitlc xsi">
				...
				<xsl:for-each select="/n1:ClinicalDocument/n1:componentOf">
					<xsl:if test="n1:encompassingEncounter">
						...
						<xsl:for-each select="/n1:ClinicalDocument/n1:componentOf/n1:encompassingEncounter/n1:id">
							<xsl:choose>
								<xsl:when test="./@root = '2.16.840.1.113883.2.1.3.2.4.18.49'">
									<tr>
										<th class="titlebar">Journey Identifier</th>
										<td class="titlebar">
	...
	<xsl:template name="history">
			...
			<xsl:if test="/n1:ClinicalDocument/n1:component/n1:structuredBody/n1:component/n1:section/n1:entry/n1:encounter">
			
			<input type="checkbox" id="toggle" />
				
				<label for="toggle">
					<span class='expand'>
						<span class="changeArrow arrow-up">↑Hide History</span>
						<span class="changeArrow arrow-dn">↓View History</span>
					</span>
				</label>		
		...
		<div class="footer">
			<table class="header">
				...
				<xsl:for-each select="n1:informationRecipient">
					<xsl:sort select="@typeCode"/>
					<xsl:if test="@typeCode='TRC'">
						<!--PRTR1 Added condition for copy recipients-->
						<tr>
							<th class="header" valign="top">
								<xsl:choose>
